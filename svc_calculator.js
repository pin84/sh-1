class Calculator {
    ...
    initUi() {
        ...
        this.fromPlace = this.rootElm.querySelector('#cal-from-place')
   
        this.fromAC = new addrTextAutoComp(this.fromPlace, (item) => {
            this.from = item
            this.showLocalTime(item)
            console.log(item);
        })

        this.toPlace = this.rootElm.querySelector('#cal-to-place')
        this.toAC = new addrTextAutoComp(this.toPlace, (item) => {
            this.to = item
            console.log(item);
        })
    ...
    async showLocalTime(obj) {
        ...
        let url = `${api[stage].maps}/timezones/location/date-time`

        let res = await svcUtils.fetchData({ url, data: { loc: `${lat},${lng}`, dt: dateStr }, isShowLoading: false })
    ...
    async timeZoneHandler(calcPostData) {
        ...
        if (!tz || flag || dayNum == 1) {
            ...
            if (!airports) return {}
            let  { lat, lng } = await airportCache(airports[0])
            let dateStr = getStandardDateTime()
            if (flag) {
    ...
    async distCalc(fromLat, fromLng, toLat, toLng) {
        ...
        let res = await svcUtils.fetchData({ url, data: d, isShowTips }) //false: don't show tips
        let { distance, time } = res
        if (!distance || !time) {
            res = {
