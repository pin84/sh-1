<template>
  <div class="item-block" ref="itemBlock">
    ...
    <div class="tc" ref="tc" :style="{transform:`translateX(${TCStyle.TCX * width / 100 - tcW - tcW / 4}px) translateY(-0.1rem)`}" :class="{'active':TCStyle.TCZT}"></div>

    <!-- <div class="zs-box" :style="TCStyle.styleZS">
    </div> -->

    <!-- TCStyle.ZSX -->
    <div class="zs" ref="zs" :style="{transform:`translateX(${TCStyle.ZSX * width / 100 - tcW - tcW / 4}px) translateY(${TCStyle.ZSY * height / 100 - zsH * 2}px)`}"  :class="{'active':TCStyle.ZSZT}"></div>
  </div>
</template>
