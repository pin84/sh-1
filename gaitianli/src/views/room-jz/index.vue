<template>
  <div class="mie-jun">
    <div class="main-body">
      <div class="box-1" :class="{'active':false}">
        <Pipe :state='f1'> </Pipe>
      </div>
      <div class="box-2" :class="{'active':false}">
        <Pipe :state='f1'></Pipe>
      </div>
  ...
  methods: {
    async initData() {
      while (true) {
        ...
        let res = await this.$get("/PeiYangGuanList/1");
        let { list } = res;
        console.log(res);
        let infoArr = [];
...
.mie-jun {
  ...
  height: 1080px;
  box-sizing: border-box;
  background: url("../../../public/gaitianli/junzhong1-4.jpg") center no-repeat;
