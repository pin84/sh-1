class VehicleMapping {
  ...
  async fillMapping() {
    ...
    mapping.forEach((obj,index)=>{
      ...
      let { elife, max } = obj
      let {vehicle_name,max_luggage,o_max_luggage} = vehicleList.find(item => item.vehicle_id == elife)
      ...
      // cItem.querySelector('.item-passanger').innerText = `${max}/${max}`
      cItem.querySelector('.item-passanger').innerText = `${maxLuggage}/${maxLuggage}`
  ...
  async show(id = 1) {
    ...
    if(dList.findIndex(item=>item.countries.includes('COMMON')) == -1){
      dList.unshift(common)
    }
