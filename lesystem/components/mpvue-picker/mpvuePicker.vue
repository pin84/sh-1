<template>
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
			...
			<!-- 动态数据三级联动 -->
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChangeMul" v-if="mode==='dynamicData' && deepLength===3">
			    <block>
			        <picker-view-column>
			            <view class="picker-item" v-for="(item,index) in pickerValueMulThreeOne" :key="index">{{item.label}}</view>
			        </picker-view-column>
    ...
    export default {
        ...
        methods: {
            initPicker(valueArray) {
                ...
                } else if (this.mode == 'dynamicData') {
					
					this.pickerValueMulArray = valueArray;
					this.pickerValueMulThreeOne = valueArray[0] != undefined ? valueArray[0] : [];
					this.pickerValueMulThreeTwo = valueArray[1] != undefined ? valueArray[1] : [];
					this.pickerValueMulThreeThree = valueArray[2] != undefined ? valueArray[2] : [];
				}
            },
            show() {
                setTimeout(() => {
            ...
            _getPickerLabelAndValue(value, mode) {
                ...
                } else if (mode == 'dynamicData'){
					pickerLable = '';
					if (this.pickerValueMulThreeOne[value[0]]) {
						pickerLable += `${this.pickerValueMulThreeOne[value[0]].label}`;
						pickerGetValue.push(this.pickerValueMulThreeOne[value[0]].value);
					}
					if (this.pickerValueMulThreeTwo[value[1]]) {
						pickerLable += `-${this.pickerValueMulThreeTwo[value[1]].label}`;
						pickerGetValue.push(this.pickerValueMulThreeTwo[value[1]].value);
					}
					if (this.pickerValueMulThreeThree[value[2]]) {
						pickerLable += `-${this.pickerValueMulThreeThree[value[2]].label}`;
						pickerGetValue.push(this.pickerValueMulThreeThree[value[2]].value);
					}
				}
                return {
