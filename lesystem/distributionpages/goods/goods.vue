<template>
	<view>
		<view class="category-content-section">
			...
			<view class="goods-list-grid open-list">
				<view class="tablelist-append">
					<view class="goods-info ns-border-color-gray" v-for="(item, index) in goodsList" :key="index">
						<view class="goods_select" v-if="item.is_select == 1">
							<text class="checkbox-old iconfont iconyuan_checked ns-text-color" @click="checkboxno()"></text>
						</view>
						<view class="goods_select" v-else>
							<text
			...
			<view style="height: 100rpx;" :class="isIphoneX ? 'iponex' : ''"></view>
			<!-- 底部添加 -->
			<view class="goods-footer ns-border-color-gray" :class="isIphoneX ? 'iponex' : ''">
				<view class="select-all">
					...
					></text>
					<view>全选</view>
					<view class="selectNum ns-text-color-black" v-if="selectCount > 0">
						已选
						<text class="base-color">{{ selectCount }}</text>
						件
					</view>
				</view>
				<button type="primary" class="add-btn" @click="addSave()">添加</button>
			</view>
		</view>
	...
	methods: {
		...
		addSave() {
			...
			if (this.isLoad) {
				...
				this.sendRequest({
					...
					success: res => {
						if (res.code == 0) {
							this.$util.showToast({ title: '添加成功' });
							this.$util.redirectTo('/distributionpages/my_goods/my_goods');
						} else {
							this.$util.showToast({ title: '添加失败' });
						}
						this.isLoad = true;
					}
				});
		...
		checkbox(index) {
			...
			this.goodsList.forEach(item => {
				...
				if (item.checked && item.is_select == 0) selectNum += 1;
			});
			this.selectCount = selectNum;
		...
		allElection(checked) {
			...
			if (this.goodsList.length) {
				this.goodsList.forEach(item => {
					...
					if (this.checkAll && item.is_select == 0) {
						this.selectCount++;
					}
				});
			}
