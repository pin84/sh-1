<template>
	<view class="page-body login_container">
		<view>
			<view class="alignFlex whiteBg">
				<text class="iconfont iconyonghu"></text>
				<input class="uni-input" v-model="submitData.UserName" maxlength="11" type="text" placeholder="请输入手机号" />
			</view>
			<view class="alignFlex whiteBg">
				<text class="iconfont iconmima"></text>
				<input class="uni-input" v-model="submitData.UserPwd" maxlength="6" type="text" password="true" placeholder="请输入密码" />
			</view>
			<view class="alignFlex whiteBg">
				<text class="iconfont iconyanzhengma"></text>
				<input class="uni-input" v-model="submitData.UserYZM" type="text" maxlength="6" placeholder="请输入验证码" />
				<button id="getCode" class="getCode" @click="getCodeClick" :disabled="submitData.UserName.length!=11||!isClick"
				 :class="submitData.UserName.length!=11||!isClick?'error':'primary'">{{btnVal}}</button>
			</view>
		</view>
		<button @click="resetClick" class="primary">重置密码</button>
		<view class="betweenFlex">
			<text @click="loginClick">立即登录</text>
			<text @click="registerClick">立即注册</text>
		</view>
	</view>
</template>

<script>
	import http from 'common/js/http.js';
	export default {
		data() {
			return {
				submitData: {
					UserName: '',
					UserPwd: '',
					UserYZM: '',
					record_id: ''
				},
				count: 60,
				isClick: true,
				btnVal: "获取验证码",
				Interval: ''
			}
		},
		methods: {
			setTime() {
				this.sendRequest({
					url: 'System.SendMsg.sendVeryCode',
					method:"POST",
					data: {
						phone: this.submitData.UserName
					},
					success: res => {
						this.submitData.record_id = res.data.record_id;
						this.Interval = setInterval(() => {
							this.count--;
							if (this.count > 0) {
								this.isClick = false;
								this.btnVal = this.count + "s后重新获取";
							} else {
								this.count = 60;
								this.btnVal = "重新发送";
								this.isClick = true;
								clearInterval(this.Interval);
							}
						}, 1000);
					}
				})
			},
			getCodeClick() {
				if (!(/^1[3|4|5|8][0-9]\d{4,8}$/.test(this.submitData.UserName))) {
					this.$util.showToast({ title: '手机号不正确' });
					return false;
				} else {
					this.setTime();
				}
			},
			resetClick() {
				if (this.submitData.UserName == "") {
					this.$util.showToast({ title: '手机号不能为空' });
					return false;
				}
				if (this.submitData.UserPwd == "") {
					this.$util.showToast({ title: '密码不能为空' });
					return false;
				}
				if (this.submitData.UserPwd.length < 6) {
					this.$util.showToast({ title: '密码不能少于6个字符' });
					return false;
				}
				if (this.submitData.UserYZM == "") {
					this.$util.showToast({ title: '请填写验证码' });
					return false;
				}
				this.reset();
			},
			reset() {
				//重置密码接口
				this.sendRequest({
					url: 'System.Login.resetPassword',
					method:"POST",
					data: {
						phone: this.submitData.UserName,
						password: this.submitData.UserPwd,
						record_id: this.submitData.record_id,
						code: this.submitData.UserYZM
					},
					success: res => {
						console.log(res);
						if (res.data.code == -1) {
							this.$util.showToast({ title: res.data.message });
							return false;
						}
						if (res.data == "") {
							this.$util.showToast({ title: '重置成功' });
							setTimeout(() => {
								this.$util.redirectTo('/pages/login/login/login', {}, '');
							}, 1500);
						}
					}
				})
			},
			loginClick(){
				this.$util.redirectTo('/pages/login/login/login', {}, '');
			},
			registerClick() {
				this.$util.redirectTo('/pages/login/register/register', {}, '');
			}
		},
		mixins: [http]
	}
</script>

<style lang="scss" scoped>
	.login_container {

		>view {
			margin: 0 auto;
			color: #aaa;
			width: 560rpx;
		}

		>view:nth-child(1) {
			padding: 40rpx 0;

			>view {
				padding: 0 30rpx;
				border: 2rpx solid #ddd;
			}

			>view:nth-child(2) {
				border-top: none;
				border-bottom: none;
			}

			button {
				width: 230rpx;
				margin: 0;
				padding: 0;
				height: 70rpx;
				line-height: 70rpx;
			}
		}

		>view:nth-child(3) {
			margin-top: 20rpx;
		}

		>button {
			width: 560rpx;
			margin: 0 auto;
			border: none;
		}

		.primary {
			background-color: #E9521D;
			color: white;
		}

		.error {
			background-color: #ddd;
			color: #aaa;
		}
	}

	.iconfont {
		font-size: 48rpx;
		color: #aaa;
	}
</style>