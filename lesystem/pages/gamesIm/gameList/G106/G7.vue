<template>
  <view class="hezhi">
    <view class="top">
      <view class="left"
        >从1-6中任选2个或多个号码，所选号码与开奖号码任意2个号码相同，即为中奖。</view
      >
...
export default {
  ...
  methods: {
    ...
    addToBasketRandom(val) {
      let { n } = val;
      let len = this.arr2.length;
      while (n--) {
        let i = this.$buyDataHandler.createRandomNum(1, len);
        let j = this.$buyDataHandler.createRandomNum(1, len);
        if (i == j) {
          j = this.$buyDataHandler.createRandomNum(1, len);
        }
    ...
    addToBasket(isShowBasket = true) {
      ...
      if (this.seletedArr2.length) {
        let obj = this.$buyDataHandler.createBuyObj();
        let { num, money } = this.$store.state.GDV3.footMoney;
        obj.len = num;
        obj.money = money;
        obj.numStr = this.seletedArr2.join("|");
        obj.orderStr = this.seletedArr2.join("|");
        this.$store.commit("v3NumBasket", obj);
      }
    ...
    selected(obj, n) {
      ...
      this.arr2.forEach((item) => {
        if (item.active) {
          seletedArr2.push(item.N);
        }
      });
      let j = seletedArr2.length;
      this.j = j;
      if (j < 2) {
        this.$store.commit("v3selectedNum", []);
        this.$store.commit("v3FootMoney", {});
    ...
    setValue() {
      ...
      let s = this.$buyDataHandler.combinatorialNumber(this.j, 2);
      let num = s;
