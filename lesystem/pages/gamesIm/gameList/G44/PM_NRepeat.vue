...
export default {
	...
	methods: {
		...
		initPage() {
			...
			let { titleArr, lenLimit, listStart, numLen, isShowTitle,positonLenLimit } = this.gameConfig;
			...
			if (JSON.stringify(selectedNum) === '{}' && basketArr.length === 0) {
				...
				if (JSON.stringify(selectedNum) !== '{}') {
					this.resultArr.forEach(str=>{
						let obj = JSON.parse(JSON.stringify(selectedNum))
						let obj1 = this.randomArr.find(item=>item.N.includes(str))
						obj.I = obj1.I
						obj.gName = obj1.N
						obj.money = obj.money / this.positonLen
						this.$store.commit('v3NumBasket', obj);
					})
				}
		...
		handlerData() {
			...
			obj.money = (obj.money / 2) * radix *this.positonLen ;
			this.$store.commit('v3FootMoney', { num: obj.len * this.positonLen, money: obj.money });
