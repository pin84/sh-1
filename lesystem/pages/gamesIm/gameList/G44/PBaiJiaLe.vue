<template>
	<view>
		<block v-for="(arr, idx) in numArr"><ListSquareCustom :title="titleArr[idx]" @selectedNum="selectedNum" :numArr="arr" /></block>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		initData() {
			...
			let arr1 = this.changeOdds(C[0].C, percentage, rate, C[0].N);
			let arr2 = this.changeOdds(C[1].C, percentage, rate, C[1].N);

			let arr3 = this.changeOdds(C[2].C, percentage, rate, C[2].N);
			let arr4 = arr2.concat(...arr3);
			
			this.numArr.push(arr1, arr4);
			
			console.log(this.numArr);
		},

		changeOdds(arr, percentage, rate, gName) {
			arr.forEach(item => {
