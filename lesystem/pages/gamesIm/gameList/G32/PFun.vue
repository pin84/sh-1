	...
	methods: {
		initPage() {
			...
			dataArr.forEach(item => {
				item.odds = this.$buyDataHandler.setNumDecimal((item.MxO - (item.MxO - item.MiO) * percentage) * rate, 3);
			});
			this.numArr = dataArr
		...
		selectedNum(){
			...
			if (i !== 0) {
				...
				let money = this.$buyDataHandler.handlerMoneyDecimal(i * radix * rate * ratio)
