<template>
  <view class="wenlu">
    <view class="title-box"><text class="title">三军 / 大小</text></view>
    <view class="lu-box">
    ...
    <view class="btn-box flex justify-between">
      <view
        class="flex btn"
        :class="{ 'btn-right': id == 1 }"
        v-for="(lt, id) in curBtnList"
        :key="id"
      >
        <view class="left">{{ lt }}</view>
        <view class="right"
          ><text
            v-for="(iconObj, idx) in iconArr"
            :key="idx"
            class="iconfont"
            :class="iconObj.name"
            :style="{ color: iconObj['color' + id] }"
          ></text
        ></view>
      </view>
    </view>
...
import canvasDataHandler from "./WenluHandlerData";
export default {
  props: {
    selectList: {
      type: Array,
