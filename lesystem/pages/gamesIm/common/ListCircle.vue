...
export default {
  ...
  methods: {
    ...
    initData() {
      let numberObj = [];
      let n = this.numLen;
      while (n-- > this.listStart) {
        if (this.isAddZero) {
          numberObj.unshift({ num: n < 10 ? "0" + n : n });
        } else {
          numberObj.unshift({ num: n });
        }
      }
      ...
      this.titleArr.forEach((item, idx) => {
        textArr.push(JSON.parse(JSON.stringify(this.textObj)));
        numArr.push(JSON.parse(JSON.stringify(numberObj)));
      });
