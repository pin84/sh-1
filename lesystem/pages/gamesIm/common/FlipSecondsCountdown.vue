...
export default {
  name: "FlipSecondsCountdown",
  props: {
    timestamps: {
      //重新启动，只watch 'deadline'还不够，因为两次时间如果一样就不会重新倒计时。
      type: Number,
      default: () => 0,
    },
    deadline: {
  ...
  created() {
    ...
    // }
    this.countdownSeconds = this.deadline;
    this.startCountdown();
  },
  mounted() {
  ...
  computed: {
    seconds() {
      return Math.trunc(this.countdownSeconds) % 60;
    },
    minutes() {
      return Math.trunc(this.countdownSeconds / 60) % 60;
    },
    hours() {
      return  Math.trunc(this.countdownSeconds / 60 / 60) % 24;
    },
    days() {
      return Math.trunc(this.countdownSeconds / 60 / 60 / 24);
    },
  ...
  methods: {
    startCountdown() {
      this.interval = setInterval(() => {
        if (this.countdownSeconds > 0) {
          ...
          if (this.countdownSeconds <= 0 || this.stop) {
    ...
    updateAllCards() {
      ...
      // if (this.days > 0) {
      //   this.timeData[0].show = true;
      //   this.updateTime(0, this.days);

      // }
      this.updateTime(1, this.hours + this.days * 24 );
