    ...
    <view class="foot flex align-center">
        ...
        <view class="icon-box" v-if="false">
          <view v-if="!isShowBasket">
            ...
            <view class="flex-sub text-center" style="flex: 0 0 100rpx">
              <i
                v-if="$store.state.GDV3.v3IsHasSelected"
                @click="clearNum"
      ...
      <view class="btn-box flex justify-around">
        ...
        <view class="btn btn-right" @click="submitSelect">
          <text v-if="isDone || !isShowBasket">{{
            !isShowBasket ? "确认选号" : "确认金额"
          }}</text>
          <text v-else @click="addOrder('1')"
            >确认下注
            <text v-if="countdown">({{ countdown }})</text>
          </text>
  ...
  methods: {
    ...
    async submitSelect() {
      ...
      if (this.isShowBasket) {
        ...
        for (let obj of arr) {
          if (obj.money === "" || !obj.money) {
            await this.$util.showConfirm({
              content: `【${obj.N || obj.gName}】金额不能为空`,
              showCancel: false,
            });
            flag = true;
            break;
        ...
        if (!flag) {
          this.$store.commit("v3BasketShowFrom", new Date().getTime());
        }
        this.isDone = flag;
        this.inputNumBasket = "";
      } else {
