<template>
  <view class="basket-list">
    ...
    <view>
      ...
      >
        <view class="flex align-center">
          <i
            @click="deleteRandom(index)"
            class="iconfont icon-guanbi margin-right-sm"
            style="font-size: 48rpx"
          ></i>
          <view class="flex flex-direction">
            <view class="flex">
              <text class="num-box text-red" style="text-align: left">{{
                item.numStr
              }}</text>
            </view>
            <view class="flex">
        ...
        <view>
          <view class="flex align-center">
            ...
            <view class="add-reducea radius flex align-center">
              <i
                class="iconfont icon-jian padding"
                @click="addReduce(item, '0')"
              ></i>
              <input
                type="number"
                :placeholder="String(item.ratio)"
                v-model="item.ratio"
                @input="inputText(item)"
                class="uni-input"
              />
              <i
                class="iconfont icon-tianjia padding"
                @click="addReduce(item, '1')"
              ></i>
            </view>
    ...
    <view class="options padding">
      ...
      <button class="cu-btn bg text-white" @click="$emit('clearAll')">
        清空
      </button>
    </view>
  </view>
...
export default {
  ...
  methods: {
    inputText(item) {
      ...
      item.money = this.$util.handlerNum(
        item.len * item.unitPrice * item.ratio,
        decimalNum
      );
