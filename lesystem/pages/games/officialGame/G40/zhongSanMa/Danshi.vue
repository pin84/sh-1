...
export default {
	...
	methods: {
		...
		addToBasket() {
			...
			let obj ,arr
			if(this.gameConfig.createBuyObject){
				obj = this.gameConfig.createBuyObject(handlerArr)
			} else {
				obj = this.createObj();
			  arr = handlerArr.split(',');
				let temArr = [];
				arr.forEach(item => {
					let tem = [];
					tem.push(item.split(''));
				...
				obj.len = arr.length;
				obj.money = this.$util.handlerMoneyDecimal(arr.length * radix * this.curRate.ratio * this.curRate.rate);
			}
			obj.I = this.curGame.C[0].I
			obj.gName = this.curGame.N
			...
			while (n--) {
				...
				if(this.gameConfig.addToBasketRandom){
				 obj = this.gameConfig.addToBasketRandom()
					...
					let str = this.$buyDataHandler.createNoReaptRandomNum(2)
		...
		createObj() {
			...
			let obj = {
				...
				orderStr: '',
				len: 1, //注数
				money: this.$util.handlerMoneyDecimal(radix * this.curRate.ratio * this.curRate.rate),
				unitPrice: radix * this.curRate.rate,
