<template>
	<view class="list-content" >
			...
			<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(value, key) in listData" :key="key" @click="goDetail(value.id)">
				<view class="uni-media-list">
					<view class="uni-media-list-body">
						<view class="uni-media-list">
						<view class="uni-media-list-title">
						<view class="media-list-content">{{ value.title }}</view>
						<view class="media-list-desc">{{value.create_time}}</view>
						</view>
		...
		<view v-if="isEmpty && listData.length == 0" class="empty">
			...
			<view class="ns-text-color-gray">暂时没有通知！</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		goDetail(id) {
			//console.log(id);
			uni.navigateTo({
				url:"/pages/article/gonggao/detail?id="+id
			});
		},
