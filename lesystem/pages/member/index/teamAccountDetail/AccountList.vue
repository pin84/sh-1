	...
	<view class="accountList">
		...
		<scroll-view class="list-view bg-white" scroll-y >
			...
			<block  v-if="accountList.selfInfo">  
				<view class="flex bg-white text-center row-line" @tap="showDetail('self')">
					<view class="flex-sub ">
					...
					<view class="flex-sub " :class="stringToFloat(accountList.selfInfo.winAndLoss,3) <0?'text-red':''">
			...
			<block  v-for="(item,index) in accountList.agentList.list"  :key="index">
				<view class="flex bg-white text-center row-line" @tap="showDetail(index)">
					...
					<view class="flex-sub " :class="stringToFloat(item.winAndLoss,3) <0?'text-red':''">
...
<!-- 			<view class="row-line no-border text-center" v-if="!accountList.listReport || accountList.listReport.length == 0">
			...
			<view class="total-bar" >
				<view class="flex  bg-grey text-center  row-line" v-if="accountList.total" @tap="showDetail('total')">
					...
					<view class="flex-sub text-white" :class="stringToFloat(accountList.total.buyAmount,3) <0?'text-red':'text-white'">
				...
				<view v-if='listDetailShow["total"]'>
					...
					<view class="flex bg-gray text-center row-line">
						...
						<view class="flex-sub " :class="stringToFloat(accountList.total.winAndLoss,3) <0?'text-red':''">
			...
			<view class="text-center text-grey margin-top-sm" v-else-if="page !== 1 || accountList.agentList.list.length == 0">
				<text>已全部加载~</text>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			stringToFloat(str, fixNum){             //设置number小数位，返回float类型
				if(str == null || str == '') {
					return 0;
				}
				str = '' + str;       //转换成字符串处理
