			...
			<view v-if="tyssOrderList.length>0" class="bg-white  margin-top solid-bottom">
				<block v-for="(item,index) in tyssOrderList" :key="index">
					...
					<view v-if="listDetailShow[index]">
						...
						<view class="flex bg-gray row-line">
							<view class="flex-sub ">
								下注内容：{{item.betContent}}
							</view>

							<view class="flex-sub ">
								比赛结果：{{item.marchResults}}
							</view>
...
<script>
	...
	import TyssScreenBar from './FbdScreenBar';
	import TyssFilterPanel from './FbdFilterPanel'
	export default {
		data() {
			return {
		...
		methods: {
			...
			searchSubmit(obj) { //提交搜索
				...
				this.requestOrderList('System.Report.getFBDBetRecord', postData, res => {
		...
		created() {
			...
			if (postData) {
				...
				this.requestOrderList('System.Report.getFBDBetRecord', {
