<template>
	<view class="filter-panel bg-white padding-sm shadow" >
		...
		<view class="flex align-center margin-top-sm">
			<text class="form-label text-right">投注类型：</text>
			<view class="position-relative flex-sub">
				<view class="border form-input ">
					<input type="text" @focus="showTypeList" @blur="hideTypeList" v-model="types" placeholder="全部"  placeholder-class="text-gray text-sub" class="text-df " />
				</view>
				
				<scroll-view scroll-y class="info-list bg-white border" v-show="typeShow">
					<view class="cu-list  menu ">
						<view class="cu-item" :class="item.value == selectedTypeId ? 'cur' : ''"
							v-for="(item,index) in typeDropItems" 
							:key="index" 
							@tap="changeTypeItem(item.value, item.name)">
							<view class="">
								{{item.name}}
							</view>
						</view>
