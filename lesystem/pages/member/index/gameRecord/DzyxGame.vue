<template>
	<view class="ZRSX">
		<DzyxScreenBar @filterTap="filterTap" @tapCancelButton='tapCancelButton' @changeStartTime="changeStartTime"
			@changeEndTime="changeEndTime" :ShowCancelButton='false' />
		<DzyxFilterPanel v-show="filterPanelShow" :listDropItems='listDropItems'
			@submit="searchSubmit" />
		<!-- 折叠面板 -->
		...
		<scroll-view class="list-view bg-white" scroll-y>
			<view v-if="dzyxOrderList.length>0" class="bg-white  margin-top solid-bottom">
...
<script>
	...
	import DzyxScreenBar from './DzyxScreenBar'
	import DzyxFilterPanel from './DzyxFilterPanel'
	export default {
		data() {
			return {
		...
		methods: {
			...
			showMore() {
				if (this.page_index < this.pageCount) {
					...
					this.requestData('System.Report.getElectronicBetRecord', postData, res => {
			...
			searchSubmit(obj) { //提交搜索
				...
				this.requestOrderList('System.Report.getElectronicBetRecord', postData, res => {
			...
			setPostData() { //生成发送数据
				let postData = {};
				let keys = ['startTime', 'endTime', 'page_index', 'page_size','isContainSub'];
		...
		created() {
			...
			if (postData) {
				...
				this.isContainSub=0
				this.requestOrderList('System.Report.getElectronicBetRecord', {
