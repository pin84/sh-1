<template>
	<view class="ZRSX">
		<ZrsxScreenBar @filterTap="filterTap" @tapCancelButton='tapCancelButton' @changeStartTime="changeStartTime"
			@changeEndTime="changeEndTime" :ShowCancelButton='false' />
		<ZrsxFilterPanel v-show="filterPanelShow" :listDropItems='listDropItems' :typeDropItems="typeDropItems"
			@submit="searchSubmit" />
		<!-- 折叠面板 -->
		...
		<scroll-view class="list-view bg-white" scroll-y>
			<view v-if="zrsxOrderList.length>0" class="bg-white  margin-top solid-bottom">
				<block v-for="(item,index) in zrsxOrderList" :key="index">
...
<script>
	...
	import ZrsxScreenBar from './ZrsxScreenBar'
	import ZrsxFilterPanel from './ZrsxFilterPanel'
	export default {
		data() {
			return {
		...
		methods: {
			...
			showMore() {
				if (this.page_index < this.pageCount) {
					...
					this.requestData('System.Report.getLiveGameBetRecord', postData, res => {
						this.zrsxOrderList.push(...res.data.list);
			...
			computeSubtotal() {
				...
				this.zrsxOrderList.forEach(item => {
			...
			searchSubmit(obj) { //提交搜索
				...
				this.requestOrderList('System.Report.getLiveGameBetRecord', postData, res => {
			...
			requestOrderList(url, data, callback) { //请求订单列表
				this.requestData(url, data, res => {
					this.zrsxOrderList = res.data.list
		...
		created() {
			...
			if (postData) {
				...
				this.requestOrderList('System.Report.getLiveGameBetRecord', {
