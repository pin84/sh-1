<template>
	<view class="ZRSX">
		<TyssScreenBar @filterTap="filterTap" @tapCancelButton='tapCancelButton' @changeStartTime="changeStartTime"
			@changeEndTime="changeEndTime" :ShowCancelButton='false' />
		<TyssFilterPanel v-show="filterPanelShow" :listDropItems='listDropItems' :ballStatusItems='ballStatus' :ballTypeItems='ballType'
			@submit="searchSubmit" />
		<!-- 折叠面板 -->
		...
		<scroll-view class="list-view bg-white" scroll-y>
			<view v-if="tyssOrderList.length>0" class="bg-white  margin-top solid-bottom">
				<block v-for="(item,index) in tyssOrderList" :key="index">
					...
					<view v-if="listDetailShow[index]">
						...
						<view class="bg-gray row-line" style="padding: 20rpx;">
							<view>
								下注内容：
							</view>
                            <view style="background-color: #f9eeee;padding: 10rpx;">
								<block v-for="(bitem,bindex) in item.betContents">
									<view style="margin-bottom: 10rpx;">
									<view v-if="bindex>0" style="border-bottom: 1px dashed #CCCCCC;"></view>
									<view>{{bitem.betContent}}</view>
									<view>{{bitem.played}}</view>
									<view>{{bitem.teamInfo}}</view>
									<view>{{bitem.leagueInfo}}</view>
								    </view>
				...
				<view class="flex bg-gray text-center row-line" @tap="showDetail('subTotal')">
					...
					<view class="flex-sub bg-black">
						<i class="iconfont iconsize icon-xiangyou text-gray"
							:class="listDetailShow['subTotal']? 'arrow-bottom':''"></i>
					</view>
				</view>
				<view class=" bg-gray" v-if="listDetailShow['subTotal']">
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							投注金额：{{subTotal.totalAmount}}
						</view>
						<view class="flex-sub ">
							有效投注：{{subTotal.totalEffectiveAmount}}
						</view>
					</view>
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							游戏盈亏：{{subTotal.totalAmount}}
						</view>
						<view class="flex-sub ">
							投注退还：{{subTotal.totalEffectiveAmount}}
						</view>
					</view>
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							中奖金额：{{subTotal.totalGameWinAmount}}
						</view>
						<view class="flex-sub ">
						</view>
					</view>
				...
				<view class="flex bg-gray text-center row-line" @tap="showDetail('total')">
					<view class="flex-sub bg-gradual-green">
						总计：
					</view>
					<view class="flex-sub bg-black ">
					...
					<view class="flex-sub bg-black">
						<i class="iconfont iconsize icon-xiangyou text-gray"
							:class="listDetailShow['total']? 'arrow-bottom':''"></i>
					</view>
				</view>
				<view class="bg-gray" v-if="listDetailShow['total']">
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							投注金额：{{total.totalAmount}}
						</view>
						<view class="flex-sub ">
							有效投注：{{total.totalEffectiveAmount}}
						</view>
					</view>
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							游戏盈亏：{{total.totalAmount}}
						</view>
						<view class="flex-sub ">
							投注退还：{{total.totalEffectiveAmount}}
						</view>
					</view>
					<view class="flex bg-gray row-line">
						<view class="flex-sub ">
							中奖金额：{{total.totalGameWinAmount}}
						</view>
						<view class="flex-sub ">
						</view>
					</view>
...
<script>
	import TyssScreenBar from './TyssScreenBar'
	import TyssFilterPanel from './TyssFilterPanel'
	...
	export default {
		...
		methods: {
			...
			showMore() {
				if (this.page_index < this.pageCount) {
					...
					this.requestData('System.Report.getSportsBetRecord', postData, res => {
						this.tyssOrderList.push(...res.data.list);
			...
			computeSubtotal() {
				...
				this.tyssOrderList.forEach(item => {
			...
			initPlatform() {
				...
				// 比赛球类和比赛状态
				this.requestData('System.Report.getBallTypeAndStatus' ,{
					gameID: -1,
				},res => {
					 this.ballType = ['全部',...Object.values(res.data.gameType)];
					 this.ballStatus = ['全部',...Object.values(res.data.gameStatus)];
				});
			},
			filterTap() { //点击筛选按钮
			...
			searchSubmit(obj) { //提交搜索
				...
				this.requestOrderList('System.Report.getSportsBetRecord', postData, res => {
			...
			requestOrderList(url, data, callback) { //请求订单列表
				this.requestData(url, data, res => {
					this.tyssOrderList = res.data.list
		...
		created() {
			...
			if (postData) {
				...
				this.requestOrderList('System.Report.getSportsBetRecord', {
						startTime:this.startTime,
						endTime:this.endTime }, res => {
				});
			}
		},
		mixins: [http]
