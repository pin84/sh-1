<template>
  <view class="ZRSX">
    ...
    <scroll-view class="list-view bg-white" scroll-y>
      <view
        v-if="dzyxOrderList.length > 0"
        class="bg-white margin-top solid-bottom"
      >
        <block v-for="(item, index) in dzyxOrderList" :key="index">
          <view
          ...
          <view v-if="listDetailShow[index]">
            <view class="flex bg-gray row-line">
              <view class="flex-sub"> 注单编号：{{ item.orderNo }} </view>
              <view class="flex-sub"> 投注时间：{{ item.betTime }} </view>
            </view>
            <view class="flex bg-gray row-line">
              <view class="flex-sub"> 游戏平台：{{ item.gameHall }} </view>
              <view class="flex-sub"> 游戏名称：{{ item.gameName }} </view>
            </view>
            <view class="flex bg-gray row-line">
              <view class="flex-sub"> 投注金额：{{ item.betAmount }} </view>
              <view class="flex-sub">
                有效投注：{{ item.effectiveAmount }}
              </view>
            </view>
            <view class="flex bg-gray row-line">
              <view class="flex-sub"> 投注退还：{{ item.waterAmount }} </view>

              <view class="flex-sub"> 中奖金额：{{ item.bonus }} </view>
            </view>
            <view class="flex bg-gray row-line">
              <view class="flex-sub"> 盈亏：{{ item.winLossAmount }} </view>
              <view class="flex-sub"> </view>
            </view>
          </view>
        </block>
        <view class="flex bg-gray text-center row-line">
          <view class="flex-sub bg-gradual-green"> 小计： </view>
        ...
        <view class="flex bg-gray text-center row-line">
          <view class="flex-sub bg-gradual-green"> 总计： </view>
          <view class="flex-sub bg-black">
...
import ByyxScreenBar from "./ByyxScreenBar";
import ByyxFilterPanel from "./ByyxFilterPanel";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    showMore() {
      if (this.page_index < this.pageCount) {
        ...
        this.requestData("System.Report.getFishBetRecord", postData, (res) => {
          this.dzyxOrderList.push(...res.data.list);
          this.computeSubtotal();
          this.$set(
            this.subTotal,
            "totalEffectiveAmount",
            this.SumeffectiveAmount
          );
    ...
    computeSubtotal() {
      ...
      this.dzyxOrderList.forEach((item) => {
    ...
    searchSubmit(obj) {
      ...
      this.requestOrderList(
        "System.Report.getFishBetRecord",
        postData,
        (res) => {
    ...
    requestOrderList(url, data, callback) {
      ...
      this.requestData(url, data, (res) => {
        this.dzyxOrderList = res.data.list;
