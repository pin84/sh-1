<template>
  <view class="accountList">
    <view class="flex bg-gray text-center row-line">
      <view class="flex-sub"> 日期 </view>
      <view class="flex-sub"> 投注金额 </view>
      <view class="flex-sub"> 中奖金额 </view>
      <view class="flex-sub"> 详情 </view>
    </view>

    <scroll-view v-if="accountLists.listReport" class="list-view bg-white" scroll-y>
      <block
        v-for="(item, index) in accountLists.listReport"
        :key="index"
      >
        <view
      ...
      <view
        ...
        v-if="!accountLists.listReport || accountLists.listReport.length == 0"
      >
      ...
      >
        <view
          ...
          v-if="accountLists.total"
          @tap="showDetail('total')"
        >
          <view class="flex-sub bg-green"> 总计 </view>
          <view class="flex-sub text-white">
            {{ stringToFloat(accountLists.total.betAmount, 3) }}
          </view>
          <view class="flex-sub text-white">
            {{ stringToFloat(accountLists.total.zhongBonus, 3) }}
          </view>
          <view class="flex-sub text-white">
            <i
        ...
        <view v-if="listDetailShow['total']">
          <view
            ...
            v-for="(item, index) in totalDetailItem"
            :key="index"
          >
            ...
            <view class="flex-sub col-2">
              {{ stringToFloat(accountLists.total[item.key], 3) }}
            </view>
          </view>
        </view>
      </view>
