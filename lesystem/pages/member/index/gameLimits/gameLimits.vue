<template>
	<view class="gamelimits">
				...
				<scroll-view scroll-y class="info-list bg-white border" v-show="listShow">
					<view class="cu-list  menu">
						<view class="cu-item" v-for="(item,index) in curCzlist" :key="index" @tap="changeItem(item.gameId, item.gameName, item.groupId)">
							<view class="">
								{{item.gameName}} [{{item.groupId == 0 ? '信' : ''}}]
							</view>
						</view>
					</view>
				</scroll-view>
		...
		<view class="flex">
			...
			<view class="flex-sub padding-sm bg-black ">单注最低</view>
			<view class="flex-sub padding-sm bg-black ">单注最高</view>
			<view class="flex-sub padding-sm bg-black ">单项最高</view>
		</view>
		<view v-if='limitsList' class="bg-white  margin-top solid-bottom">
			<view v-for="(item,index) in limitsList" :key="index" class="flex border-bottom" style="width:100%">
				<view class="flex-sub padding-sm ">{{item.groupName}}</view>
				<view class="flex-sub padding-sm ">{{parseFloat(item.itemhighest)}}</view>
				<view class="flex-sub padding-sm ">{{parseFloat(item.lowest)}}</view>
				<view class="flex-sub padding-sm ">{{parseFloat(item.highest)}}</view>
			</view>
		</view>
		<view v-else>
	...
	export default {
		...
		methods: {
							...
							for (let k = 0; k < seclist.length; k++) {
								...
								if (seclist[k].channelCode == 'CPYX') {
									for (let m = 0; m < seclist[k].gameInfoList.length; m++) {
										//取【信】数据
										if (seclist[k].gameInfoList[m].groupId == 0) {
			...
			getLimitsData() {
				...
				this.requestData('System.Member.getGameLimits', postData, res => {
					this.limitsList = res.data
				});
			},
