...
export default {
  ...
  methods: {
    ...
    to_register: function () {
	  ...
	  this.info.originalInviteCode=this.inviteCode;
      //console.log(this.info);
      this.$post("Pc.Login.generalizeRegisteredMemberAdd", this.info).then((res) => {
    ...
    // 获取注册项数据
    load_regist_List() {
      this.$get("Pc.MemberAdregister.generalizeRegistered",{inviteCode:this.inviteCode}).then((res) => {
		  if(res.code>=0){
  ...
  created() {
    ...
    if (!this.$isEmpty(this.$route.query.inviteCode)) {
