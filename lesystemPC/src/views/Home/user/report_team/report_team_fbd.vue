  ...
  <div class="search_content">
    ...
    <div class="result_content">
      ...
      >
        ...
        <el-table-column label="充值" width="128" prop="rechargeAmount">
        </el-table-column>
        <el-table-column label="提款" width="128" prop="withdrawAmount">
        </el-table-column>
        <el-table-column label="投注人数" width="128" prop="buyNumber">
        </el-table-column>
        <el-table-column label="投注金额" width="128" prop="betAmount" sortable>
        </el-table-column>
        <el-table-column
          label="有效投注"
          width="128"
        ...
        <el-table-column label="盈亏" width="128" prop="winLossAmount" sortable>
          <template slot-scope="scope">
        ...
        <el-table-column label="盈利总和" width="128" prop="profit" sortable>
          <template slot-scope="scope">
            <span :class="scope.row.profit < 0 ? 'text-red' : ''">{{
              scope.row.profit
            }}</span>
          </template>
        </el-table-column>
        <el-table-column label="实货量" width="128" prop="realAmount" sortable>
          <template slot-scope="scope">
            <span :class="scope.row.realAmount < 0 ? 'text-red' : ''">{{
              scope.row.realAmount
            }}</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="total total1" v-if="total_lsit">
        <el-row no-gutter>
          <el-col class="el-cola" :span="24">
            ...
            <span>{{ total_lsit.winLossAmount }}</span>
            <span>{{ total_lsit.profit }}</span>
            <span class="lastone">{{ total_lsit.realAmount }}</span>
          </el-col>
        </el-row>
      </div>
...
export default {
  ...
  methods: {
    ...
    getNowTime() {
      ...
      var defaultDate = `${year}-${month}-${date}${hour}`; //当日开始时间
      var defaultDate2 = `${year}-${month}-${date}${endhour}`; //当日结束时间
    ...
    onSerachLiveGames() {
      ...
      this.$get("Pc.Agent.getTeamFbdGamesReport", {
