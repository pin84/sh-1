  ...
  <div class="search_content">
    <el-form :inline="true" :model="info">
      ...
      <el-form-item label="球类名称">
        <el-select
          v-model="info.current_ballType"
          placeholder="全部"
          @focus="getBallTypeAndStatus($event)"
        >
          <el-option
            v-for="(value, key, index) in ballType"
            :label="value"
            :value="key"
            :key="index"
          ></el-option>
      ...
      <el-form-item label="注单状态">
        ...
        >
          <el-option
            v-for="(value, key, index) in ballStatus"
            :label="value"
            :value="key"
            :key="index"
          ></el-option>
    ...
    <div class="result_content">
      ...
      >
        <el-table-column label="订单编号" width="110" prop="orderNo">
        </el-table-column>
        <el-table-column label="投注时间" width="120" prop="betTime">
        </el-table-column>
        <el-table-column label="游戏平台" width="70" prop="gamePlatform">
        </el-table-column>
		<el-table-column label="类型" width="70" prop="ballType">
		</el-table-column>
		
        <el-table-column label="用戶" width="80" prop="username">
        </el-table-column>
        <el-table-column label="盘口" width="90" prop="oddDesc">
        </el-table-column>
        <el-table-column label="赔率" width="80" prop="odds">
        </el-table-column>
        <el-table-column label="下注内容" width="350" prop="betContents">
			<template slot-scope="scope">
				<div v-for="(item,index) in scope.row.betContents">	
				    <div v-if="index>0" style="border-bottom: 1px dashed #CCCCCC;"></div>
				   <div><span>{{item.betContent}}</span></div>
				   <div><span style="color: #0077AA">{{item.played}}</span></div>
				   <div><span style="color: #FA658D">{{item.teamInfo}}</span></div>
				   <div><span style="color: #0000FF;">{{item.leagueInfo}}</span></div>
				  
				</div>
			</template>
        </el-table-column>
        <el-table-column label="注单状态" width="100" prop="orderStatusL">
        </el-table-column>
        <el-table-column label="投注金额" width="100" prop="betAmount">
        </el-table-column>
        <el-table-column label="有效投注" width="100" prop="effectiveAmount">
        </el-table-column>
        <el-table-column label="中奖金额" width="100" prop="bonus">
        </el-table-column>
        <el-table-column label="盈亏" width="100" prop="winLossAmount">
        </el-table-column>
      </el-table>
      <div class="total" v-if="this.orderlist.length > 0">
        <el-row>
          ...
          <el-col :span="4"
            >投注额度<span>{{ subTotal.totalAmount }}</span></el-col
          >
          <el-col :span="4"
          ...
          >
          <el-col :span="4"
            >投注退还：<span>{{ subTotal.totalWaterAmount }}</span></el-col
          >
          <el-col :span="4"
        ...
        <el-row>
          ...
          <el-col :span="4"
            >投注额度<span>{{ total.totalAmount }}</span></el-col
          >
          <el-col :span="4"
          ...
          >
          <el-col :span="4"
            >投注退还：<span>{{ total.totalWaterAmount }}</span></el-col
          >
          <el-col :span="4"
  ...
  methods: {
    ...
    onsearch_order() {
      ...
      let d = {
        ...
        ballType: this.info.current_ballType,
        status: this.info.current_status || -1,
      ...
      this.$get("Pc.Report.getSportsBetRecord", d).then((res) => {
        if (res.code == 0) {
    ...
    // 球类名称 注单状态
    getBallTypeAndStatus() {
	  if(this.info.current_platform<=0){
		  this.$message.error("先选择游戏平台");
	  }	
	  else{
		  this.$get("Pc.Report.getBallTypeAndStatus", {
		    gameID: this.info.current_platform,
		  }).then((res) => {
		    if (res.code == 0) {
		      //console.log(res.data);
		      //this.ballType = Object.values(res.data.gameType);
			  this.ballType=res.data.gameType;
			  //console.log(this.ballType);
		      //this.ballStatus = Object.values(res.data.gameStatus);
			  this.ballStatus = res.data.gameStatus;
			  //this.ballStatus.unshift({-1:"全部"});
		    } else {
		      return this.$message.error(res.message);
		    }
		  });
