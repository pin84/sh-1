<template>
  <div class="user_content_blcok">
    <div class="user_content_blcok_title" style="position: relative">
      {{ title }}<span
        ><el-button size="mini" type="danger" @click="get_add_cash_info()"
          >申请提现</el-button
        ></span
      >
    </div>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="'申请提现'">
      <div class="home_form_main">
        <el-form label-width="100px" ref="info">
          <el-form-item label="余额" prop="now_money"
            ><el-input disabled v-model="info.money"></el-input
          ></el-form-item>
          <el-form-item label="手续费率" prop="rate"
            ><el-input disabled v-model="info.cash_rate"
              ><template slot="append">%</template></el-input
            ></el-form-item
          >
          <el-form-item label="银行名称" prop="rate"
            ><el-input disabled v-model="info.bank_name"></el-input
          ></el-form-item>
          <el-form-item label="银行卡号" prop="rate"
            ><el-input disabled v-model="info.card_no"></el-input
          ></el-form-item>
          <el-form-item label="真名" prop="rate"
            ><el-input disabled v-model="info.real_name"></el-input
          ></el-form-item>
          <el-form-item
            label="提现金额"
            prop="money"
            ...
            ><el-input placeholder="0.00" v-model="money"></el-input
          ></el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    get_cash_log: function () {
      this.$post(this.$api.homeGetCashLog, { page: this.current_page }).then(
        (res) => {
    ...
    add_cash: function () {
      this.$post(this.$api.homeAddCashLog, { money: this.money }).then(
        (res) => {
          if (res.code == 200) {
