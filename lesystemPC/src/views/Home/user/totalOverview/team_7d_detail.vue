<template>
  <div>
    ...
    <el-dialog
      title="7天未登录人数明细"
      :visible.sync="team7d_Visable"
      width="60%"
    >
      <el-table
        :data="team7d_list.list"
        border
        style="width: 100%"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="team7d_Visable = false"
          >关闭</el-button
        >
      </span>
    </el-dialog>
  ...
  methods: {
    ...
    */
    getAgentRechargeList() {
      this.$get("Pc.Agent.getMore7daysUnloginMemberList", {
        accountID: this.$parent.accountID,
        page: this.currentPage,
        page_size: this.page_size,
      }).then((res) => {
        if (res.code == 0) {
          this.team7d_list = res.data;
          this.total_count = Number(res.data.total_count);
          // console.log(this.team7d_list);
        } else {
