<template>
  <div class="zhenghe">
    <div v-for="(item, index) in list" :key="index" style="margin-top: 1px">
      <LongHuList :isShowInput="isShowInput" itemWidth="14%" :list="item" />
    </div>
  </div>
</template>
<script>
...
export default {
  data() {
    return {
      ...
      // list3: [],
      // list2: [],
      titleList: ["正一", "正二", "正三", "正四", "正五", "正六", "正七"],
      titleList1: ["前三", "中三", "后三"],
  ...
  methods: {
    ...
    initData() {
      ...
      let AllArr = this.$store.state.gd.curGame.C;
      let { percentage } = this.$store.state.gd.footOdds;

      let list = [];
      this.titleList.forEach((str) => {
        list.push(AllArr.filter((item) => item.N.startsWith(str)));
      });

      this.changeOddsOption(list, percentage);
      this.list = list;
    },
