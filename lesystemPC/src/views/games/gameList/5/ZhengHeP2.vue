<template>
  <div class="zhenghe">
    <div class="list">
      <div class="item" v-for="(title, index) in len" :key="index">
        <ZhengHeList
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      let temArr = [];
      for (let item of arr) {
        if (item.Id == 651 || item.Id == 207 || item.Id == 165) {
          continue;
        }
        item.C.forEach((obj2) => {
          obj2.N1 = item.N;
          obj2.odds = obj2.MxO - (obj2.MxO - obj2.MiO) * percentage;
          temArr.push(obj2);
        });
      }

      let aa = this.$BDH.arrayDivision(temArr, 4);
      aa.forEach((arr, index) => {
        if (list[index]) {
          list[index].push(...arr);
        }
      });
