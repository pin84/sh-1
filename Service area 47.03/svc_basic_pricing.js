...
class BasicPricing extends Container {
  constructor(id = 'basic-pricing') {
    super();
    ...
    this.raisingcardsNum = this.rootElm.querySelector('.raisingcards-num');
    this.raisingCardsNumBox = this.rootElm.querySelector('.raising-cards-price-box');
    this.changeUrbanBtn = this.rootElm.querySelector('#change-urban-area-btn');
    this.changeSvcPriceBtn = this.rootElm.querySelector('#change-svc-area-btn');
    this.tabUnderline = this.rootElm.querySelector('.tab-underline');
    ...
    calcBtn.onclick = function () {
      ...
      } else if (calcCount <= it.defaultData.min_distance) {
        resultBox.textContent = it.defaultData.min_amount;
        console.log('resultBox.textContent_2', resultBox.textContent);
      } else {
        resultBox.textContent = (it.defaultData.min_amount - 0 + it.calcFee(calcCount - it.defaultData.min_distance)).toFixed(2);
  ...
  initElms() {
    ...
    it.lowPriceEnd.value = this.defaultData.min_distance;
    if (it.changeType == 1) {
      it.raisingcardsNum.value = this.defaultData.meet_n_greet || '';
    }
    ...
    rangeUnits.forEach((item) => {
      // item.textContent = this.defaultData.perUnit == 1 ? 'M' : 'KM'
      item.textContent = it.countryIm == 1 ? 'KM' : 'M';
    });
    ...
    it.lowPriceEnd.onblur = function (e) {
      it.defaultData.min_distance = e.target.value;
    ...
    it.startPrice.addEventListener('input', function (e) {
      it.defaultData.min_amount = e.target.value - 0;
    });
    it.raisingcardsNum.addEventListener('input', function (e) {
      it.defaultData.meet_n_greet = e.target.value - 0;
    });
    ...
    for (let obj of getData) {
      ...
      (function (k) {
        ...
        unitInput.onblur = function (e) {
          ...
          it.defaultData.ranges[k].amount = e.target.value - 0;
          it.checkRangeNumFn();
          it.initElms();
        };
        // && getData.length > 1
    ...
    it.addRangeBtn.onclick = function () {
      ...
      let newItem = {
        ...
        distance: '0',
        from: index == 0 ? it.defaultData.min_distance : it.defaultData.ranges[index - 1].to,
        to: '',
        distanceIn: 0,
        per_unit: it.countryIm,
      };
    ...
    it.changeTypeBtns = it.rootElm.querySelectorAll('.range-type-btn');

    it.changeSvcPriceBtn.onclick = () => {
      if (serviceArea.downtown_areas?.area_geometry?.length && !it.checkRangeNumFn()) {
        it.initElms();
        return false;
      }
      it.pour(serviceArea);
      it.changeType = 1;
      it.changeUrbanBtn.classList.remove('range-type-btn-activy');
      it.changeSvcPriceBtn.classList.add('range-type-btn-activy');
      // Service Area
      it.tabUnderline.classList.remove('tab-underline-2');
      // it.raisingCardsNumBox.classList.remove('hide')
      setTimeout(() => {
        it.show();
      }, 300);
    };
    it.changeUrbanBtn.onclick = () => {
      if (!it.checkRangeNumFn()) {
        it.initElms();
        return false;
      }
      it.pour(serviceArea);
      it.changeType = 2;
      it.changeSvcPriceBtn.classList.remove('range-type-btn-activy');
      it.changeUrbanBtn.classList.add('range-type-btn-activy');
      // Downtown Area
      it.tabUnderline.classList.add('tab-underline-2');
      // it.raisingCardsNumBox.classList.add('hide')
      setTimeout(() => {
        it.show();
      }, 300);
    };
  ...
  checkRangeNumFn() {
    ...
    if (this.defaultData.ranges && this.defaultData.ranges.lenth != 0) {
      this.defaultData.ranges.forEach((item, index) => {
        ...
        item['amountActivy'] = Boolean(item.amount < 0);
        item['fromActivy_2'] = !Boolean(item.from - 0);
        item['toActivy_2'] = !Boolean(item.to - 0);
      });
      return this.defaultData.ranges.every((item) => item.amount >= 0 && item.from > 0 && item.to > 0 && item.from < item.to) && priceFlag;
    } else {
      return false;
    }
  }
  pour(serviceAreaData) {
    ...
    let { ranges, min_distance, currency } = pourData;
    pourData.min_distance = mkm2mtr(min_distance, this.countryIm);
    pourData.currency = serviceAreaData.currencyCode;
    if (ranges) {
      for (let item of ranges) {
        let to = mkm2mtr(item.to, this.countryIm);
        let from = mkm2mtr(item.from, this.countryIm);
        item.to = to;
        item.from = from;
        item.currency = serviceAreaData.currencyCode;
        item.distance = to - from;
        item.per_unit = this.countryIm;
    ...
    if (this.changeType == 1) {
      serviceAreaData.pricing = pourData;
    } else {
      serviceAreaData.downtown_areas = pourData;
    }
  }
  show() {
    let it = this;
    ...
    let cData = null;
    it.changeRangeType = it.rootElm.querySelector('.change-range-type');
    if (!downtown_areas || !downtown_areas.area_geometry) {
      it.changeRangeType.classList.add('hide');
      it.changeType = 1;
      it.changeUrbanBtn.classList.remove('range-type-btn-activy');
      it.changeSvcPriceBtn.classList.add('range-type-btn-activy');
      it.tabUnderline.classList.remove('tab-underline-2');
      ...
      it.changeRangeType.classList.remove('hide');
    }
    if (it.changeType == 1) {
      cData = pricing ? JSON.parse(JSON.stringify(pricing)) : null;
    } else {
      cData = downtown_areas?.ranges?.length ? JSON.parse(JSON.stringify(downtown_areas)) : null;
    }
    if (cData) {
      cData.min_distance = mtr2mkm(cData.min_distance, it.countryIm);
      let { ranges } = cData;
      if (ranges) {
        cData.ranges.forEach((item, index) => {
          cData.ranges[index].from = mtr2mkm(item.from, it.countryIm);
          cData.ranges[index].to = mtr2mkm(item.to, it.countryIm);
          delete item.amountActivy;
          delete item.fromActivy;
          delete item.fromActivy_2;
          delete item.toActivy;
        ...
        cData.ranges = [
          {
            ...
            to: '',
            distanceIn: 0,
            per_unit: this.countryIm,
          },
        ];
      }
      // if (cData.ranges.length) {
      ...
      cData = {};
      if (it.changeType == 2 && downtown_areas?.area_geometry) cData.area_geometry = downtown_areas.area_geometry;
      cData.ranges = [
        {
          seq: 0,
          amount: '0',
      ...
      if (it.changeType == 1) {
        cData.meet_n_greet = 0;
      }
      cData.min_amount = 0;
      cData.min_distance = 0;
      this.defaultData = JSON.parse(JSON.stringify(cData));
      this.initElms(true);
    }
