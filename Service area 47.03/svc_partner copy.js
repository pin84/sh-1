class Partner {
  ...
  resetPageData() {
    ...
    this.fillPartnerList(this.selectedPartner ? [this.selectedPartner] : _p.partnerList);
  ...
  showVehicleMapping(id, wrapper) {
    let x = wrapper.offsetLeft + 15
    let y = wrapper.offsetTop + 20 - this.scrollTop
    vmMapping.show(id, x, y)
  }

  findPartnerId(elm) {
    let id = elm.getAttribute('partner_id')
    if (!id) {
      elm = elm.parentNode
      id = this.findPartnerId(elm)
    }
    return id
  }
  ...
  async partnerListClickEvent() {
    ...
    this.partnerListBox.addEventListener('click', async (e) => {
      let target = e.target;
      let partnerId = this.findPartnerId(target)
      let toIndex = target.getAttribute('to-index') || target.parentNode.getAttribute('to-index')

      if (!toIndex) return;
      if (toIndex == 5) {
        this.showVehicleMapping(partnerId, target.parentNode)
      } else {
        if (toIndex == 4) {
        ...
        let curPartnerData = _p.partnerList.find((item) => item.partner_id == partnerId);
        _p['curPartnerData'] = JSON.parse(JSON.stringify(curPartnerData));

        console.log("---_p['curPartnerData'] ---", _p['curPartnerData'] );
        ...
        switch (toIndex) {
          ...
          case 3:
            ...
            this.amenitySvcs = uniqueArr
            this.createSetPriceList(uniqueArr);
            this.initSetVehicleClassBox(this.svcList);
            this.svpcSaveBottomBox.classList.remove('hide');
          ...
          if (item) {
            if (this.svcType == 3) {
              ...
              if (index == -1) {
                ...
                this.amenitySvcs.push(item)
                this.createSetPriceList(this.amenitySvcs, item.service_area_id);
  ...
  createSetPriceList(list, showId = 0) {
    ...
    if (countriesList.length !== 0) {
      let index = this.svcNameElms.findIndex(item => item.id == showId)
      if (index == -1) {
        index = 0
      }
      this.svcNameElms[index].elm.click();
    }
  }
  ...
  countrySvcListClickEvent() {
    this.countrySvcList.addEventListener('click', async (e) => {
      ...
      if (e.target.tagName == 'SPAN') {
        ...
        this.svcNameElms[0]&& this.svcNameElms[0].elm.click();
  ...
  fillPartnerList(list = []) {
    ...
    for (let obj of list) {
      let { partner_id, fleet_id, partner_name_svc } = obj
      let cItem = this.partnerItemBox.cloneNode(true);
      cItem.classList.remove('hide', 'template');
      let partnerId = cItem.querySelector('.item-partner-id');
      partnerId.innerText = partner_id;

      cItem.setAttribute('partner_id', partner_id)
      // cItem.setAttribute('fleet-id', fleet_id)


      let partnerName = cItem.querySelector('.item-partner-name');
      partnerName.innerText = partner_name_svc;
      ...
      settingBtns.forEach((elm, index) => {
        ...
        // elm.setAttribute('partner-id', partner_id);
        // elm.setAttribute('fleet-id', fleet_id);
      });
      // let vmBox = cItem.querySelector('.vm')
      // vmBox.setAttribute('toIndex', partner_id)
      framgent.appendChild(cItem);
    }
  ...
  async getPartnerList() {
    ...
    let { results } = await svcUtils.fetchData({
      url,
      method: 'POST',
      data: postData
    })
    ...
    _p.partnerList = this.dataHandler(results);
    console.log('----------- _p.partnerList-----', _p.partnerList);
  }
  ...
  async saveData() {
    ...
    this.svpcSabeBtn.classList.add('active');
    let { total_affected_rows } = res;
    if (total_affected_rows >= 2) {
      svcUtils.toast();
      await this.getPartnerList();
      _p['curPartnerData'] = _p.partnerList.find((item) => item.partner_id == _p['curPartnerData'].partner_id);

      console.log('--save--curPartnerData-', _p['curPartnerData']);
      if (this.selectedPartner) {
        this.selectedPartner = _p['curPartnerData'];
      }
      let index = _p.partnerList.findIndex((item) => item.partner_id == _p['curPartnerData'].partner_id);
      _p.partnerList[index] = _p['curPartnerData'];
      this.partnerDropdown.setItems(_p.partnerList);
    } else {
      svcUtils.msgBox({
  ...
  initPartnerSearchBox() {
    ...
    let dropdown = new TextAutoComp(input, {
      ...
      sel: (item, hovElm) => {
        if (item) {
          ...
          input.value = '';
          this.fillPartnerList(_p.partnerList);
          this.selectedPartner = null;
        }
      },
    });
    ...
    dropdown.setItems(_p.partnerList);
  }
